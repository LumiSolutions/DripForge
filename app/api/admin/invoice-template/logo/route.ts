...
export async function POST(request: Request) {
  ...
  try {
    ...
    const current = await getInvoiceTemplateSettings()
    const saved = await saveInvoiceTemplateSettings({
      ...current,
      logoUrl: url,
    ...
    console.error("Admin: Rechnungslogo-Upload fehlgeschlagen.", error)
    return NextResponse.json(
      { error: "Rechnungslogo konnte nicht gespeichert werden." },
      { status: 500 }
    )
  }
}
