...
export function buildIncomeStatement(
  ...
  return [
    { id: "ertrag", label: "Betrieblicher Ertrag aus Lieferungen und Leistungen (Klasse 3)", amount: ertrag, level: 1 },
    { id: "material", label: "Aufwand für Material, Handelswaren, Dienstleistungen (Klasse 4)", amount: -material, level: 1 },
    { id: "brutto", label: "= Bruttoergebnis", amount: brutto, level: 0, emphasis: true },
    { id: "personal", label: "Personalaufwand (Klasse 5)", amount: -personal, level: 1 },
    { id: "uebrig", label: "Übriger betrieblicher Aufwand (Klasse 6)", amount: -uebrig, level: 1 },
    { id: "betrieb", label: "= Betriebsergebnis (EBIT)", amount: betrieb, level: 0, emphasis: true },
    { id: "finanz", label: "Finanzerfolg (Klasse 8)", amount: roundChf(finanz - finanzAufwand), level: 1 },
    { id: "ausser", label: "Ausserordentlicher Erfolg (Klasse 9)", amount: ausserord, level: 1 },
    { id: "erfolg", label: "= Unternehmenserfolg (Gewinn / Verlust)", amount: erfolg, level: 0, emphasis: true },
  ]
}
...
function balanceForClass(
  ...
  for (const entry of entries) {
    ...
    for (const line of entry.lines) {
      if (accountClass(line.accountNumber) !== classDigit) continue
      const amount = roundChf(line.amount)
      if (side === "aktiv") {
        total += line.type === "SOLL" ? amount : -amount
      } else {
...
export function buildBalanceSheet(
  ...
  return [
    { id: "aktiven-h", label: "AKTIVEN", amount: aktivenTotal, level: 0, section: "aktiven", emphasis: true },
    { id: "umlauf-h", label: "Umlaufvermögen", amount: umlauf, level: 1, section: "aktiven" },
    { id: "fluessig", label: "Flüssige Mittel", amount: fluessig, level: 2, section: "aktiven" },
    { id: "forderungen", label: "Forderungen", amount: forderungen, level: 2, section: "aktiven" },
    { id: "anlage-h", label: "Anlagevermögen", amount: anlage, level: 1, section: "aktiven" },
    { id: "aktiven-t", label: "Total Aktiven", amount: aktivenTotal, level: 0, section: "aktiven", emphasis: true },
    { id: "passiven-h", label: "PASSIVEN", amount: passivenTotal, level: 0, section: "passiven", emphasis: true },
    { id: "kurz-h", label: "Kurzfristiges Fremdkapital", amount: kurzFk, level: 1, section: "passiven" },
    { id: "lang-h", label: "Langfristiges Fremdkapital", amount: langFk, level: 1, section: "passiven" },
    { id: "eigen-h", label: "Eigenkapital", amount: eigen, level: 1, section: "passiven" },
    { id: "passiven-t", label: "Total Passiven", amount: passivenTotal, level: 0, section: "passiven", emphasis: true },
  ]
}
